...
const Followers = () => {
  ...
  useEffect(() => {
    ...
    const predicate = () => {
      return hasMoreFollowers && !loadFollowerListLoading;
    };

    const onScroll = scrollHandler(
      dispatch.bind(null, loadFollowersAction({ userId: id, lastId: lastId })),
      predicate
    );
  ...
  return (
    <AppLayout pageName={userInfo.nickname}>
      ...
      {FollowerList.length === 0 ? (
        ...
        FollowerList.map((user) => <FollowCard key={user.id} user={user} />)
      )}
      {loadFollowerListLoading ? (
        <MiddleWrapper>
          <LoadingIcon />
